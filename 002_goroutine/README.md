...
- Fan-Out : 논리회로에서 주로 쓰이는 요어로 게이트 하나의 출력이 게이트 여러 입력으로 들어가는 경우를 팬아웃이라고 합니다.
...
이 채널에서 값이 오기를 기다리는지에 상관 없이 모두 이것을 알게 됩니다. 따라서 채널을 닫는 것은 신호를 모두에게 저달하기 위한 매우 강력하고 깔끔한 방법입니다. 
